...
class GMTool {
    ...
    def setConfig(def statistics = null, String username = null, String password = null, def licenseServer = null,
        ...
        if (username != null && password != null) {
            command.add(OPT_USERNAME_CONFIG + username)
            command.add(OPT_PASSWORD_CONFIG + password)
        ...
        if (licenseServerAddress != null) {
            throwIfNotCompatible(GMToolFeature.Feature.ONSITE_LICENSE_CONFIG, "config $OPT_LICENSE_SERVER_ADDRESS") {
                command.add(OPT_LICENSE_SERVER_ADDRESS + licenseServerAddress)
            }
        }
        if (proxy != null) {
            command.add(OPT_PROXY + proxy)
        }
        if (proxyAddress != null) {
            command.add(OPT_PROXY_ADDRESS + proxyAddress)
        }
        if (proxyPort != null) {
            command.add(OPT_PROXY_PORT + proxyPort)
        }
        if (proxyAuth != null) {
            command.add(OPT_PROXY_AUTH + proxyAuth)
        }
        if (proxyUsername != null) {
            command.add(OPT_PROXY_USERNAME + proxyUsername)
        }
        if (proxyPassword != null) {
            command.add(OPT_PROXY_PASSWORD + proxyPassword)
        }
        if (virtualDevicePath != null) {
            command.add(OPT_VIRTUAL_DEVICE_PATH + virtualDevicePath)
        }
        if (androidSdkPath != null) {
            command.add(OPT_SDK_PATH + androidSdkPath)
        }
        if (useCustomSdk != null) {
            command.add(OPT_USE_CUSTOM_SDK + useCustomSdk)
        }
        if (screenCapturePath != null) {
            command.add(OPT_SCREEN_CAPTURE_PATH + screenCapturePath)
        }
